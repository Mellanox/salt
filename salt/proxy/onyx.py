...
def add_config(lines):
    ...
    .. note::
        For more than one config added per command, lines should be separated by coma.
        salt '*' onyx.cmd add_config 'vlan 2, exit, interface ethernet 1/1 switchport access vlan 2'
    '''
    ...
    try:
        ...
        for line in lines:
            ...
            if "Unrecognized command" in out:
                configure_terminal_exit()
                disable()
                return 'Failed to run command "%s" on minion \nError:\n"%s"' % (line, out)
